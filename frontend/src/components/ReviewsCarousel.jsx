...
export const ReviewsCarousel = () => {
  ...
  useEffect(() => {
    let mounted = true;
    axios.get(`${API}/reviews?status=approved`).then((res) => {
      if (!mounted || !res.data?.length) return;
      const userReviews = res.data.map((r) => ({
        name: r.name, org: r.org, rating: r.rating, text: r.text, date: r.date, photo: r.photo_data_url,
      }));
      // Merge user reviews first, then static
      setAllReviews([...userReviews, ...REVIEWS]);
    }).catch(() => {});
            ...
            <div key={dup} className="flex gap-6 shrink-0">
              {allReviews.map((r, i) => (
                ...
                >
                  ...
                  {r.photo && (
                    <img src={r.photo} alt="" className="w-full h-32 object-cover rounded mb-3 border border-white/10" />
                  )}
